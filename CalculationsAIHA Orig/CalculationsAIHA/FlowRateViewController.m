...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    FlowRateManager *FlowRatemanager = [FlowRateManager sharedFlowRateManager];
    
    return [FlowRatemanager.formulas count];
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    static NSString *CellIdentifier = @"FlowRateCell";
    ...
    NSDictionary *formulaInfo = [FlowRatemanager.formulas objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    FlowRatemanager.selectedFormula = [FlowRatemanager.formulas objectAtIndex:indexPath.row];
    
    NSDictionary *chosenFormula = FlowRatemanager.selectedFormula;
    
    int Name =[[chosenFormula objectForKey:@"name"] intValue];
    
    CategoryManager *catManager = [CategoryManager sharedCategoryManager];    
    catManager.category = 13;
